...
def checkdir():
    for folder in ['WRDS/Database', 'WRDS/output_direct', 'WRDS/output_dropna',
                   'WRDS/output_dropna/simple', 'WRDS/output_dropna/smart', 'WRDS/output_dropna/smartexmom']:
...
def green2output():
    ...
    chars = pd.read_csv(os.path.join('WRDS/Database', '_characteristics.csv'), encoding='iso-8859-1')[
        'name'].tolist()
    necessary_cols = [item.lower()
                      for item in chars.copy() + ['gvkey', 'DATE', 'prc']]

    df = pd.read_parquet(os.path.join('WRDS/Database', 'green.parquet'))
    df = df[df.columns[df.columns.str.lower().isin(necessary_cols)]]
    df = df.dropna(subset=['gvkey', 'DATE']).drop(
        index=163354)  # somehow that row is duplicated
    df['DATE'] = pd.to_datetime(
        df['DATE'], format='%Y%m%d').dt.strftime('%Y-%m-%d')

    # pivot the price dataframe
    prc_df = df[['gvkey', 'DATE', 'prc']]
    prc_df = prc_df.pivot(index='DATE', columns='gvkey',
                          values='prc').sort_index()
    ...
    """
    mom_df.to_csv(os.path.join('WRDS/output_direct',
                  '_momentum.csv'), index=True)
    del mom_df
    ...
    for date in all_dates.iloc[1:]:
        ...
        # YYYY-(MM-48)-DD to YYYY-(MM-1)-DD
        t48m_prc_df = prc_df.iloc[max(todays_index - 48, 0):todays_index]
        momm_df = t48m_prc_df.iloc[-1]/t48m_prc_df - 1  # mom_n where n > 1
        momm_df.loc[last_month] = prc_df.iloc[todays_index] / \
            prc_df.loc[last_month] - 1  # mom_1

        # rename columns
        ind_dict = pd.Series(momm_df.index).sort_values(
        ...
        charm_df.to_csv(os.path.join(
            'WRDS/output_direct', date + '.csv'), index=True)

        print('-', end='')
        print(f']\n{int(date[:4])+1} [', end='') if '-12-' in date else None

    print('\n\nOverall NaN ratio:', round(
        df.isna().sum().sum() / df.size * 100, 2), '%\n\n')
    del df, prc_df


def direct2dropnas():
    start_year = c.listdir('WRDS/output_direct')[0][:4]
    print(f'Dropping NaNs\n{start_year} [', end='')
    for afile in c.listdir('WRDS/output_direct', '.csv'):
        if afile.startswith('_'):
            continue
        df = pd.read_csv(os.path.join(
            'WRDS/output_direct', afile)).set_index('gvkey')
        df_simple = df.dropna(axis=1, how='any')
        df_simple.to_csv(os.path.join(
            'WRDS/output_dropna', 'simple', afile), index=True)
        df_smart = c.smartdropna(df)
        df_smart.to_csv(os.path.join(
            'WRDS/output_dropna', 'smart', afile), index=True)
        df_smart2 = c.smartdropna(
            df, exclude=['mom_'+str(i) for i in range(1, 49)])
        df_smart2.to_csv(os.path.join('WRDS/output_dropna',
                         'smartexmom', afile), index=True)

        print('-', end='')
        print(f']\n{int(afile[:4]) + 1} [',
              end='') if '-12-' in afile else None
