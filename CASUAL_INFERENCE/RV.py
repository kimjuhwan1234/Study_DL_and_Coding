...
try:
    close_df_sorted = pd.read_csv(
        'data/ETF_price.csv', index_col=0, parse_dates=True)
    print(f"데이터 로드 완료: {close_df_sorted.shape}")
    print(f"컬럼: {list(close_df_sorted.columns)}")
except FileNotFoundError:
    print("ETF_price.csv 파일을 찾을 수 없습니다. 먼저 EDA.ipynb를 실행하여 데이터를 생성해주세요.")
    exit()
...
realized_vol_3d.columns = [col.replace(
    '_close', '_RV_3d') for col in realized_vol_3d.columns]
realized_vol_15d.columns = [col.replace(
    '_close', '_RV_15d') for col in realized_vol_15d.columns]
realized_vol_1m.columns = [col.replace(
    '_close', '_RV_1m') for col in realized_vol_1m.columns]
...
# realized_vol_3d.to_csv('data/realized_volatility_3d.csv', encoding='utf-8')
# realized_vol_15d.to_csv('data/realized_volatility_15d.csv', encoding='utf-8')
# realized_vol_1m.to_csv('data/realized_volatility_1m.csv', encoding='utf-8')

# images 폴더 생성 및 모든 종목 쌍의 그래프 저장

# images 폴더 구조 생성
# vol_types = ['RV_3d', 'RV_15d', 'RV_1m']
# vol_dataframes = [realized_vol_3d, realized_vol_15d, realized_vol_1m]

# # 종목명 추출 (컬럼명에서 _RV_ 부분 제거)
# tickers = [col.replace('_RV_3d', '').replace('_RV_15d', '').replace('_RV_1m', '')
